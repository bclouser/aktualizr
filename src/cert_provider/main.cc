...
int main(int argc, char* argv[]) {
  ...
  if (commandline_map.count("credentials_path") != 0) {
    credentials_path = commandline_map["credentials_path"].as<boost::filesystem::path>();
  }

  if (local_dir.empty() && target.empty()) {
    std::cerr << "Both a local dir and a target are not specified, thus the resultant cert and key will be lost" << std::endl;
  }

  std::string serverUrl;
  if ((fleet_ca_path.empty() || provide_ca || provide_url) && credentials_path.empty()) {
    std::cerr << "Error: missing -c/--credentials parameters which is mandatory if the fleet CA is not specified or an output of the root CA or a gateway URL is requested";
    return EXIT_FAILURE;
  } else {
  ...
  try {
    ...
    if (!local_dir.empty()) {
      std::cout << "Writing the generated and signed client certificate and key to " << local_dir << " ...\n";
